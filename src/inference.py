...
def convert_example_to_features(example, tokenizer, max_seq_length, id = 0, no_ins_at_first = False, tokenizing = False):
    ...
    no_ins_array = np.full(max_seq_length, dtype=int, fill_value=-1)
...
def greedy_search(model, input_ids, segment_ids, input_mask, no_ins = None, device='cuda', temperature=1.0, tokenizer=None, prevent=None, promote=None, reduce=None, verbose = None):
    zero_list = ["[", "]", "(", ")"]
    ...
    for ip in range(MAX_TURN):
        with torch.no_grad():
            ...
            if reduce:
                reduce_factor = min(float(ip), 1.0) 
                for p in reduce:
...
def sample_generate(model, input_ids, segment_ids, input_mask, no_ins = None, device='cuda', temperature=1.0, args=None, tokenizer=None, sample_num=1, top_k=10, top_p=0.9, prevent=None, promote=None, reduce=None, verbose = None):
    ...
    for ip in range(MAX_TURN):
        with torch.no_grad():
            ...
            logits[:,:, zero_ids] = -1e10
            for i in range(256):
                val = top_k_top_p_filtering(logits[:,i].clone().squeeze(), top_k = top_k, top_p = top_p)
                logits[:,i] = val
            probs = F.softmax(logits, dim=-1)
